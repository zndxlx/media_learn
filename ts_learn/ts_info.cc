...
*/
#if 1
// Transport Stream packets are 188 bytes in length.  ts包大小固定为188 bytes
...
enum TSPidTable
{
    ...
    TSPidTablePAT = 0x00, //pid 为0 表示pat
...
/*ts 头中 adaptation_field_control, 所以ts头后面可能有一个 adaptation_field*/
/**
...
// Table 2-29 – Stream type assignments. page 66. 流的数据类型，在pmt表中stream_type有用，标识一个pid对应的码流类型
enum TSStreamType
{
...
/**
* the actually parsed type.  测试分析用的，不是标准中的定义，用于记录pid对应类型
*/
enum TSPidType
{
...
// TSPacket declares.  定义中各ts包，包含 adaption_field, 内容， adaption_field是可能有的
class TSPacket
{
public:
    TSHeader *header;
    ...
    TSPacket();
    virtual ~TSPacket();
    //ctx 解包上下文， start 开始指针， last 结束指针，p 指向当前解包指针
    int demux(TSContext *ctx, u_int8_t *start, u_int8_t *last, u_int8_t *&p, TSMessage *&pmsg);
    int finish();
};

/**
* 2.4.3.2 Transport Stream packet layer. page 36. 定义ts头
*/
class TSHeader
{
public:
    // 1B
    int8_t sync_byte; //8bits  固定为0x47 同步字节
    // 2B
    int8_t transport_error_indicator;    //1bit
    int8_t payload_unit_start_indicator; //1bit  负载单元起始标识，一个完整的数据包开始时标记为1
    int8_t transport_priority;           //1bit   传输优先级，0为低优先级，1为高优先级，通常取0
    TSPidTable pid;                      //13bits   pid值, 很重要 可以判断这个包是否是pat包
    // 1B
    int8_t transport_scrambling_control;   //2bits   传输加扰控制，00表示未加密
    ...
    //ctx 解包上下文，pkt指向当前的ts包，start ts包头， last ts包尾
...
/**
* 2.4.4.3 Program association Table. page 61.   pat表结构
*/
class TSPayloadPAT
{
public:
    // 1B
    u_int8_t table_id; //8bits   PAT表固定为0x00

    // 2B
    int8_t section_syntax_indicator; //1bit  固定为1
    int8_t const0_value;             //1bit   固定为0
    // 2bits reserved.
    u_int16_t section_length; //12bits   后面数据的长度

    // 2B
    u_int16_t transport_stream_id; //16bits   传输流ID，固定为0x0001

    // 1B
    // 2bits reerverd.
    int8_t version_number;         //5bits   版本号，固定为00000
    int8_t current_next_indicator; //1bit   固定为1，表示这个PAT表可以用，如果为0则要等待下一个PAT表

    // 1B
    u_int8_t section_number; //8bits    固定为0x00

    // 1B
    u_int8_t last_section_number; //8bits  固定为0x00
    ...
    // 4B
    int32_t CRC_32; //32bits   前面数据的CRC32校验码
...
public:
    // 1B
    u_int8_t stream_type; //8bits 流类型，标志是Video还是Audio还是其他数据，h.264编码对应 0x1b，aac编码对应0x0f，mp3编码对应0x03

    // 2B
    // 3bits reserved
    int16_t elementary_PID; //13bits   与stream_type对应的PID
...
public:
    // 1B
    u_int8_t table_id; //8bits  PMT表取值随意，0x02
    ...
    // 2B
    u_int16_t program_number; //16bits  表示当前的PMT关联到的频道

    // 1B
    // 2bits reerverd.
    int8_t version_number;         //5bits   版本号，固定为00000
    int8_t current_next_indicator; //1bit   固定为1，表示这个PMT表可以用
    ...
    char *program_info_desc;     //[program_info_length]bytes  节目描述信息，指定为0x000表示没有
...
public:
    // 3B
    int32_t packet_start_code_prefix; //24bits  开始码，固定为0x000001
    // 1B
    u_int8_t stream_id; //8bits TSPESStreamId   音频取值（0xc0-0xdf），通常为0xc0 视频取值（0xe0-0xef），通常为0xe0
    // 2B
    u_int16_t PES_packet_length; //16bits  pes数据的长度，0表示长度不限制，只有视频数据长度会超过0xffff
    ...
    // 1B
    u_int8_t PES_header_data_length; //8bits  后面数据的长度

    int64_t pts; // 33bits   显示时间戳
    int64_t dts; // 33bits    解码时间戳
...
public:
    ...
    * consumed pids.
    */
    int pid_size; //TSPid列表长度
    TSPid *pids;  //TSPid列表
    int64_t ts_packet_count;
    std::map<TSPidTable, TSMessage *> msgs; //pid-->msg
...
int TSPayloadPAT::demux(TSContext *ctx, TSPacket *pkt, u_int8_t *start, u_int8_t *last, u_int8_t *&p, TSMessage *&pmsg)
{
    ...
    // 4 is crc size.  根据section_length可以计算节目表的个数
...
int TSPayloadPES::demux(TSContext *ctx, TSPacket *pkt, u_int8_t *start, u_int8_t *last, u_int8_t *&p, TSMessage *&pmsg)
{
    ...
    if (stream_id != PES_program_stream_map && stream_id != PES_padding_stream && stream_id != PES_private_stream_2 && stream_id != PES_ECM_stream && stream_id != PES_EMM_stream && stream_id != PES_program_stream_directory && stream_id != PES_DSMCC_stream && stream_id != PES_H_222_1_type_E)
    {
        original_or_copy = *p++;
    ...
    else if (stream_id == PES_program_stream_map || stream_id == PES_private_stream_2 || stream_id == PES_ECM_stream || stream_id == PES_EMM_stream || stream_id == PES_program_stream_directory || stream_id == PES_DSMCC_stream || stream_id == PES_H_222_1_type_E)
    {
...
int TSPayload::demux(TSContext *ctx, TSPacket *pkt, u_int8_t *start, u_int8_t *last, u_int8_t *&p, TSMessage *&pmsg)
{
    ...
    if (pkt->header->pid == TSPidTablePAT) //pat包
    {
    ...
    if (pid && (pid->type == TSPidTypeVideo || pid->type == TSPidTypeAudio))
    {
        ...
        if (msg->packet_start_code_prefix == 0x01 && pkt->header->payload_unit_start_indicator == 1 && msg->PES_packet_length == 0)
        {
            msg->detach(ctx, pmsg);
        ...
        // parse continous packet. ts标识这个不是一个完整包(pes)的开始
        if (!pkt->header->payload_unit_start_indicator)
        {
            ...
            msg->append(p, last - p); //将包体内容拷贝到msg中
        ...
        type = pid->type;
        pes = new TSPayloadPES(); //在头开始时候这个时候解pes相关的信息,
...
int TSPacket::demux(TSContext *ctx, u_int8_t *start, u_int8_t *last, u_int8_t *&p, TSMessage *&pmsg)
{
    ...
    // calc the user defined data size for payload. 计算payload的大小
    payload->size = TS_PACKET_SIZE - header->get_size() - adaption_field->get_size();

    //解ts payload, 注意TSAdaptionTypeAdaptionOnly不需要解ts payload
...
int TSHeader::demux(TSContext *ctx, TSPacket *pkt, u_int8_t *start, u_int8_t *last, u_int8_t *&p, TSMessage *&pmsg)
{
    ...
    // 同步到上下文 pid标识包类型(0表示是pat)  有什么用？统计吗
...
int main(int argc, char **argv)
{
    ...
    {
        u_int8_t ts_packet[TS_PACKET_SIZE]; //ts包为固定188byte大小
        ...
        while (p == start)
        {
            ...
            offset += nread;
            if (!msg) //如果解出一个完整的包,msg会指向该包
            {
                continue;
            }

            if ((ret = consume(msg, &aac_muxer)) != 0) //将包写入aac文件
            {
